...
def test_with_gaussian_noise(model, X_test, y_test, tree_types, noise_levels, n_realizations=1000):
    """
    Тестирование с гауссовским шумом - МУЛЬТИПЛИКАТИВНАЯ методология
    """
    print("\n" + "="*70)
...
def print_confusion_matrix_table(results, tree_types):
    ...
    """
    print("\n" + "="*90)
    print("📊 МАТРИЦЫ ОШИБОК В ТАБЛИЧНОМ ФОРМАТЕ")
    print("="*90)
    ...
    for noise_level in target_noise_levels:
        if noise_level in results:
            print(f"\n🔊 МАТРИЦА ОШИБОК ДЛЯ ШУМА {noise_level*100:.0f}%")
            print("-" * 70)
            ...
            for i, tree in enumerate(tree_types):
                ...
                for j in range(len(tree_types)):
                    if i == j:  # Диагональные элементы (правильная классификация)
                        row += f" | {cm[i,j]:>8.2f}"
                    else:  # Ошибки классификации
                        if cm[i,j] > 0:
                            row += f" | {cm[i,j]:>8.2f}"
                        else:
                            row += f" | {'0':>8}"
