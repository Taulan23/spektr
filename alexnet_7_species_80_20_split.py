...
def main():
    """Основная функция с правильным разделением 80/20"""
    print("ОБУЧЕНИЕ МОДЕЛИ С РАЗДЕЛЕНИЕМ 80/20")
    print("=" * 60)
    ...
    print("\nСоздание улучшенной модели 1D-CNN...")
    ...
    plot_confusion_matrix(
        ...
        f"Матрица ошибок 1D-CNN (7 видов, 80/20 разделение)",
        f"alexnet_7_species_80_20_confusion_matrix_{timestamp}.png"
    )
    
    # График истории обучения
    plot_training_history(
        history,
        f"alexnet_7_species_80_20_training_history_{timestamp}.png"
    )
    
    # Сохраняем модель и препроцессоры
    model.save(f'alexnet_7_species_80_20_model_{timestamp}.h5')
    
    import joblib
    joblib.dump(scaler, f'alexnet_7_species_80_20_scaler_{timestamp}.pkl')
    joblib.dump(label_encoder, f'alexnet_7_species_80_20_label_encoder_{timestamp}.pkl')
    ...
    noise_df.to_csv(f'noise_test_results_{timestamp}.csv', index=False, encoding='utf-8-sig')
    
    print(f"\nМодель и результаты сохранены с временной меткой: {timestamp}")
    print("\nАнализ 1D-CNN для 7 видов (80/20 разделение) завершен!")
    
    # Открываем результаты
    import subprocess
    subprocess.run(['open', f'alexnet_7_species_80_20_confusion_matrix_{timestamp}.png'])
    subprocess.run(['open', f'alexnet_7_species_80_20_training_history_{timestamp}.png'])
