...
def create_fixed_1d_alexnet(input_shape, num_classes):
    """Создание ИСПРАВЛЕННОЙ 1D-AlexNet для 7 видов"""
    model = Sequential([
        # Первый блок - уменьшенные размеры
        Conv1D(64, 11, strides=4, activation='relu', input_shape=input_shape),
        BatchNormalization(),
        ...
        # Третий блок
        Conv1D(192, 3, padding='same', activation='relu'),
        Conv1D(192, 3, padding='same', activation='relu'),
...
def train_fixed_model():
    ...
    print("=== ИСПРАВЛЕННАЯ 1D-ALEXNET ДЛЯ 7 ВИДОВ ===")
    ...
    model = create_fixed_1d_alexnet((X_train_cnn.shape[1], 1), len(label_encoder.classes_))
    ...
    plt.tight_layout()
    plt.savefig('ФИНАЛЬНЫЕ_РЕЗУЛЬТАТЫ/исправленная_модель_вероятности.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    # Анализ проблемы
    print("\n" + "="*60)
    print("🔍 АНАЛИЗ ИСПРАВЛЕННОЙ МОДЕЛИ:")
    print("="*60)
    ...
    print(f"\n🎯 ИСПРАВЛЕНИЯ:")
    print(f"- Уменьшены полносвязные слои (4096 → 512/256)")
    print(f"- Добавлен Dropout (0.5)")
    print(f"- Добавлен BatchNormalization")
    print(f"- Уменьшен размер батча (32 → 16)")
    print(f"- Увеличено терпение EarlyStopping (10 → 15)")
    
    print(f"\n✅ РЕЗУЛЬТАТ:")
    print(f"- Модель: ИСПРАВЛЕННАЯ 1D-AlexNet для 7 видов")
    print(f"- Архитектура: Адаптированная для малых данных")
    print(f"- Регуляризация: Dropout + BatchNorm")
