...
def load_spectral_data_7_species():
    ...
    for species_name in species:
        ...
        # Берем первые 20 файлов для каждого вида (уменьшено для скорости)
...
def create_confusion_matrices(results, y_test, label_encoder, noise_levels):
    ...
    plt.savefig(f'simple_7_species_confusion_matrices_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def analyze_7_species_classification():
    ...
    # Обучаем модель (уменьшено количество эпох для скорости)
    print("Обучение модели...")
    history = model.fit(
        X_train_reshaped, y_train,
        epochs=20,  # Уменьшено с 50 до 20
        batch_size=32,
    ...
    model.save(f'simple_7_species_model_{timestamp}.h5')
    
    import joblib
    joblib.dump(scaler, f'simple_7_species_scaler_{timestamp}.pkl')
    joblib.dump(label_encoder, f'simple_7_species_label_encoder_{timestamp}.pkl')
    ...
    joblib.dump(results_summary, f'simple_7_species_results_{timestamp}.pkl')
    ...
    plt.title('Точность модели (7 видов)')
    plt.xlabel('Эпоха')
    plt.ylabel('Точность')
    plt.legend()
    ...
    plt.title('Ошибка модели (7 видов)')
    plt.xlabel('Эпоха')
    plt.ylabel('Ошибка')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(f'simple_7_species_training_history_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.ylabel('Точность')
    plt.title('Точность модели 1D-CNN для 7 видов при разных уровнях шума')
    ...
    plt.tight_layout()
    plt.savefig(f'simple_7_species_noise_accuracy_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    print(f"\nАнализ упрощенной 1D-CNN для 7 видов завершен!") 